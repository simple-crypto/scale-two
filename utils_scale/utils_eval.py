...
def display_rank_esti_full_key(uni_TA_results, multi_TA_results, byte_indexes, key_rank_approximation_scalib, title2=None):
    # Unpack the results
...
def display_rank_esti_full_key_axes(TA_results, byte_indexes, key_rank_approximation_scalib):
    ### 
    ax_sx_inch = 5
    ax_sy_inch = 3
    figsize=(2*ax_sx_inch, 1*ax_sy_inch)
    f = plt.figure(figsize=figsize)
    axes = [f.add_subplot(1,len(TA_results),1+i) for i in range(len(TA_results))]

    for ax, TAres in zip(axes, TA_results):
        # Unpack the results
        (mTA_probs, _, _, mTAqas, mTAkbytes) = TAres
        ...
        test_scale.display_rank_esti_full_key_axe(
                ax,
                (bridge_TA_res_probs_25_to_24(mTA_probs), mTAkbytes, np.array(list(mTAqas))),
                byte_indexes,
                key_rank_approximation_scalib
...
def explore_params(pi_LDA_multi_TA, ti_LDA_multi_TA, traces, labels, ntraces_pi, npois, ndims, qp=None, nclasses=256, chunk_size=1000):
    # Some size fetching
    ...
    with tqdm.tqdm(total=len(npois)*len(ndims), desc="param sets") as pbar:
        for i_npois, e_npois in enumerate(npois):
            for i_ndims, e_ndims in enumerate(ndims):
                # PI
                wrap_pi_LDA_multi_TA = lambda a,b,c,d: pi_LDA_multi_TA(a, b, c, d, e_npois, e_ndims, nclasses=nclasses, chunk_size=chunk_size)
                ...
                # TI
                wrap_ti_LDA_multi_TA = lambda a,b: ti_LDA_multi_TA(a, b, e_npois, e_ndims, nclasses=nclasses, chunk_size=chunk_size)            
